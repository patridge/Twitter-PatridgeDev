...
function renderResults() {
  const output = results.map(item => `<p class="search_item"><div class="search_text">${item.full_text}</div><div class="search_time">${new Date(item.created_at).toLocaleString()} <div class="search_link"><a class="permalink" href="PatridgeDev/status/${item.id_str}">🔗</a>${item.is_reply?'<div class="reply_badge">(reply)</div>':""}</div></div><hr class="search_divider" /></p>`.replace(/\.\.\/\.\.\/tweets_media\//g,'PatridgeDev/tweets_media/'));
...
function renderBrowse() {
  const output = browseDocuments.slice(browseIndex, browseIndex + pageSize).map(item => `<p class="search_item"><!-- Avatar here --><div class="search_text">${item.full_text}</div><div class="search_link"><div class="search_time">${new Date(item.created_at).toLocaleString()} <a class="permalink" href="PatridgeDev/status/${item.id_str}">🔗</a>${item.is_reply?'<div class="reply_badge">(reply)</div>':""}</div></div><hr class="search_divider" /></p>`.replace(/\.\.\/\.\.\/tweets_media\//g,'PatridgeDev/tweets_media/'));
